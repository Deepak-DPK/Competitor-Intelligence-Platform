      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200/70">
        <div>
          <h2 className="text-lg font-bold text-slate-900 tracking-tight flex items-center gap-2">
            ...
            <span>Settings & Crawler Pipeline</span>
          </h2>
          <p className="text-xs text-slate-500 mt-1">
            Configure Playwright headless browser runners, Jina Reader clean markdown scrapers, and Gemini API models.
          </p>
        </div>
      </div>
        ...
        <Card>
          <CardHeader>
            <div>
              ...
              <CardDescription>Headless browser automation for rate scraping & screenshot diffs</CardDescription>
            </div>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-9 px-3 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-slate-900"
                >
                  <option value="2 Nodes">2 Parallel Containers</option>
                  <option value="4 Nodes">4 Parallel Containers (Recommended)</option>
          ...
          <CardHeader>
            <div>
              <CardTitle className="flex items-center gap-2">
                ...
                Jina Reader & Gemini AI Model Integration
              </CardTitle>
              <CardDescription>Clean Markdown HTML rendering and intelligence generation</CardDescription>
            </div>
          </CardHeader>

          <div className="space-y-4 mt-3">
            <div className="p-3 bg-slate-900 text-slate-200 rounded-xl text-xs font-mono space-y-1">
              <div>Gemini AI Model: {systemStatus?.geminiModel || 'gemini-2.5-flash'}</div>
              <div>Jina Reader Latency: {systemStatus?.jinaReaderLatencyMs || 340}ms</div>
              <div>Environment Variable: GEMINI_API_KEY (Server-Side Injected)</div>
            </div>
          </div>
        </Card>
